...
public class Grid {
		...
		try {
			while ((nextLine = reader.readNext()) != null) {
				for (j = 0; j < nextLine.length; j++) {
					...
					break;
					case ROCK: grid[i][j].rocktype[level] = new LithologicType(Integer.parseInt(value));
					break;
	...
	public void setRock(Point loc, int[] rocks) {
		LithologicType[] lithRocks = new LithologicType[grid[loc.y][loc.x].layers.length];
	 ...
	 * @param array Strings to extract from
	...
	/**Sets the main array of rocks, as well as the rocks on each
	 * individual cell.
	 ...
	 * @param x is the cell's row location
	 * @param y  is the cell's col location
	 * @param depth is the layer's depth (layer 0, 1, 2, etc) 
	 ...
	 * 
	 * @param p is the point of the cell
	...
	public int getSurfaceLayer(Point p){
		int k=0;
		try{for(k=0;grid[p.y][p.x].layers[k]==null;k++);}
		catch(Exception e){ //This will occur when the map is initialized, because it doesn't yet have cell info.
		}
		return k;
	}
